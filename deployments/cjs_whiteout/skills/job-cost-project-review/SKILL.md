...
5. Normalize equivalent numeric formatting but report every real numeric mismatch, including zero and small decimal differences. Count a numeric mismatch only when both source values are numeric and differ after normalization. A blank, `None`, dash, missing value, or uncalculated value on either side is not a numeric mismatch. Exclude it from the mismatch count and mention it separately only when missing-value reporting is requested.
