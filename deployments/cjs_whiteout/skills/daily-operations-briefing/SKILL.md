...
6. Name an owner only when a source explicitly assigns responsibility for the next action. A recipient, sender, addressee, original requester, person who placed an earlier order, or person merely named in a forwarded thread is not the owner. `Addressed to Mike` and `original order was from Alyssa` do not assign ownership. Otherwise say only `Owner not specified`. Never add who would typically handle it.
