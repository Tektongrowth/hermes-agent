...
13. When someone asks for dashboard job totals or profit alongside labor, call `synkedup_job_costing` with no query for every included job or exactly `status:completed` for completed rows. This one tool already includes estimated and actual labor. Do not also call `synkedup_labor_variance` for the same request. For a comparison with a native Google Sheet, find the exact file with `composio_search`, then call `composio_read_drive_spreadsheet` with that file ID. Do not substitute `composio_execute` for the spreadsheet reader. Match jobs by exact job number. For the `2026 Project Review Sheet`, compare every matched row one field at a time across Estimated Hours, Actual Hours, Final Net Profit %, Final Net Profit $, and Final Total. Report every mismatch in those fields, including zero values and small decimal differences. Do not count the project-name column as a mismatch unless the requester asks to compare names. Before answering, verify that every completed SynkedUP job was either matched to the sheet or listed as missing, and that every reported mismatch includes both source values. Do not stop after saying what you will compare.
